...
def process_url(url: str, request_id: str) -> str:
    """
    Process and validate an image URL.
    If the URL is a data URL, extract the base64 content and save it to a temporary file.
    """
...
def classify_image():
    ...
    try:
        ...
        # Handle URL input
            ...
            logger.info(f"Processing URL input: {url[:100]}...")
            try:
                processed_url = process_url(url, request_id)
                # If it's a local file (from a data URL)
                if Path(processed_url).is_file():
                    try:
                        result = classifier.classify_image(processed_url)
                        return jsonify(create_response(result, 'url', request_id, original_url=url))
                    finally:
                        cleanup_image(processed_url)
                # Otherwise, treat it as a regular URL and download the image
                else:
                    temp_path = upload_dir / f'temp_url_{request_id}.jpg'
                    try:
                        ...
                        with open(temp_path, 'wb') as f:
                            f.write(image_data)
                        result = classifier.classify_image(temp_path)
                        return jsonify(create_response(result, 'url', request_id, processed_url=processed_url))
                    finally:
                        cleanup_image(temp_path)
            except Exception as e:
        ...
        # Handle file upload
            ...
            try:
                ...
                try:
                    ...
                    return jsonify(create_response(result, 'file', request_id, filename=file.filename))
                finally:
            ...
            return jsonify({'error': 'No image provided', 'request_id': request_id}), 400
...
def request_entity_too_large(error):
    ...
    return jsonify({
        ...
        'details': {'max_size_mb': config.MAX_CONTENT_LENGTH / (1024 * 1024)}
    }), 413
